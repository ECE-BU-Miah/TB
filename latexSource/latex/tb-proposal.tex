...
\begin{figure}
  ...
  \begin{tikzpicture}    
      ...
      (sum.-165)node[left]{$+$};
      \draw[latex-]
      (sum3.west)-- node[midway,above]{$x_2^{[d]}$}++(-4*\smgrid,0)node[anchor=east]{Target flow rate};
      \draw
      (sum3.-165)node[left]{$+$};
...
\begin{figure}
  ...
  \begin{tikzpicture}    
      ...
      \node[sum](sum){$\sum$};
      \node[block, text width=2 cm, text centered, right of = sum, node distance=3 cm](D1){Droplet Size (Pressure) Controller $D_1(z)$};
      \node[block, text width=1 cm, text centered, right of = D1, node distance=4.5 cm](G){Plant $G(z)$};      
      \node[block, text width=1.5 cm, text centered, below of = D1, node distance=3.0 cm](H1){Pressure sensor $H_1(z)$};
      \node[sum, right of = H1, node distance=2 cm](sum1){$\sum$};      
      ...
      (sum.east) -- node[midway,above]{$e_1$}(D1.west);
      \draw[-latex]
      (D1.east) -- node[near start,above]{$u_1$} (G.west);
      ...
      ($(D1.north)+(\smgrid,\smgrid)$)node[]{\textcolor{blue}{{\bf Discrete-time control system}}};
    \end{tikzpicture}  
  \caption[One-input, One-output discrete-time control system block diagram.]{One-input, One-output discrete-time control system block diagram.}
...
A typical pressure range is around 55 to 80psi.
A typical flow rate range is around 5 to 15 gallons per acre.

\todo[inline]{OPTIONAL: description of $x_1,$ $x_2, \ldots$}
